...
ae2f_SHAREDEXPORT void ResRoomLobby(sock_t sock, const uSockAddr *addr,
    ...
    if (req && req->m_req == REQ_ROOMLOBBY && addr &&
      ...
      __RoomLobby(req->m_room, &v_retroom, req->m_rname, req->m_rpwd, sock,
                  addr, req->m_plname);
      if (sizeof(globplayer_t) == sendto(sock, (const void*)&v_retroom, sizeof(v_retroom), 0, &addr->m_addr,
                                   SockAddrLen)) {
      }
    }  
}
...
ae2f_SHAREDCALL
void ResRoomShow(sock_t clisock, const uSockAddr* cliaddr, __ReqRoomShowBuf* req) {
  if((req) && (cliaddr) && clisock != INVALID_SOCKET) {
    ...
    if(req->pad + req->count <= MAX_ROOM_COUNT) {
      ...
      ssize_t a = sendto(clisock, ae2f_reinterpret_cast(const void*, Rooms + req->pad), sizeof(Room) * req->count, 0, &cliaddr->m_addr, SockAddrLen);
      dbg_printf("sendto: %d\n", a);
    }
  }
}
